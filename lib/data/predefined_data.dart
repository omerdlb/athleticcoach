import 'package:athleticcoach/data/models/test_definition_model.dart';

final List<TestDefinitionModel> predefinedTests = [
  // Aerobik Testler
  TestDefinitionModel(
    id: 'yo-yo-ir1',
    name: 'Yo-Yo Intermittent Recovery Test Level 1',
    category: 'Aerobik',
    description: 'Yo-Yo IR1, 20 metrelik parkurda, artan hızda gidip gelmeler ve her 40 metrede 10 saniye aktif dinlenme içeren, aralıklı egzersiz kapasitesini ölçen bir saha testidir. Özellikle futbol, basketbol gibi takım sporlarında yaygın olarak uygulanır.',
    purpose: 'Sporcunun hem aerobik dayanıklılığını hem de yüksek şiddetli tekrarlar arası toparlanma kapasitesini ölçer. Takım sporcularında maç temposuna uyum, antrenman programı değerlendirmesi ve VO2max tahmini için kullanılır.',
    protocol: 'Parkur Hazırlığı:\n1. 20 metrelik düz bir parkur hazırlanır, iki uca işaret konur.\n2. Sinyal ses kaydı ve kronometre hazır edilir.\n\nUygulama:\n1. Katılımcı, sinyal sesiyle bir uçtan diğer uca koşar.\n2. Her mekik sonrası 10 saniye aktif dinlenme (yavaş yürüyüş) yapılır.\n3. Sinyale yetişilemezse uyarı verilir, üst üste iki kez yetişilemezse test biter.\n4. Test boyunca sesli sinyal kaydı ve kronometre kullanılır.\n\nSonuç ve Puanlama:\n- Son tamamlanan seviye kaydedilir (her seviye 8 mekik içerir).\n- Toplam koşulan mesafe = (seviye sayısı) × 8 × 20 metre.\n- VO2max tahmini için: VO2max = (mesafe × 0.0084) + 36.4',
    resultUnit: 'Seviye',
  ),
  TestDefinitionModel(
    id: 'yo-yo-ir2',
    name: 'Yo-Yo Intermittent Recovery Test Level 2',
    category: 'Aerobik',
    description: 'Daha yüksek tempoda tekrarlı sprint ve aerobik kapasiteyi ölçer.',
    purpose: 'Sporcunun yüksek şiddetli tekrarlar arası toparlanma kapasitesini ölçer. Takım sporlarında maç performansını öngörmede ve antrenman programlarını değerlendirmede kullanılır.',
    protocol: 'Parkur Hazırlığı:\n1. 20 metrelik düz bir parkur hazırlanır, iki uca işaret konur.\n2. Sinyal ses kaydı ve kronometre hazır edilir.\n\nUygulama:\n1. Katılımcı, sinyal sesiyle bir uçtan diğer uca koşar.\n2. Her mekik sonrası 10 saniye aktif dinlenme (yavaş yürüyüş) yapılır.\n3. Başlangıç hızı ve artışlar Level 1\'e göre daha yüksektir.\n4. Sinyale yetişilemezse uyarı verilir, üst üste iki kez yetişilemezse test biter.\n\nSonuç ve Puanlama:\n- Son tamamlanan seviye kaydedilir (her seviye 8 mekik içerir).\n- Toplam koşulan mesafe = (seviye sayısı) × 8 × 20 metre.\n- VO2max tahmini için: VO2max = (mesafe × 0.0136) + 45.3',
    resultUnit: 'Seviye',
  ),
  TestDefinitionModel(
    id: 'beep-test',
    name: '20m Shuttle Run (Beep Test)',
    category: 'Aerobik',
    description: 'VO2max tahmini için en yaygın saha testlerinden.',
    purpose: 'VO2max tahmini için en yaygın saha testlerinden.',
    protocol: 'Parkur Hazırlığı:\n1. 20 metrelik düz bir parkur hazırlanır, iki uca işaret konur.\n2. Sinyal ses kaydı ve kronometre hazır edilir.\n\nUygulama:\n1. Katılımcı, sinyal sesiyle bir uçtan diğer uca koşar.\n2. Her sinyalde bir uçtan diğer uca ulaşmak zorundadır.\n3. Başlangıçta tempo yavaştır, her dakika hız artar (sinyaller arasındaki süre kısalır).\n4. Katılımcı sinyale yetişemezse uyarılır, üst üste iki kez yetişemezse test sonlanır.\n\nSonuç ve Puanlama:\n- Son tamamlanan seviye kaydedilir (ör: 8.5 = Seviye 8\'de 5. mekikte bitti).\n- Toplam mesafe = (seviye × 20m).\n- VO2max tahmini için: VO2max = (seviye × 3.5) - 3.5 (ml/kg/dk)',
    resultUnit: 'Seviye',
  ),
  TestDefinitionModel(
    id: 'cooper',
    name: 'Cooper Testi (12 Dakika Koşu)',
    category: 'Aerobik',
    description: 'Cooper testi, 12 dakika boyunca düz bir pistte veya ölçülü bir alanda mümkün olan en uzun mesafeyi koşmaya dayalı klasik bir saha dayanıklılık testidir.',
    purpose: 'Aerobik kapasiteyi (VO2max) pratik ve hızlı şekilde tahmin etmek, sporcuların genel dayanıklılığını ve antrenman düzeyini değerlendirmek için kullanılır.',
    protocol: 'Parkur Hazırlığı:\n1. Düz bir atletizm pisti veya ölçülü bir alan hazırlanır.\n2. Kronometre ve ölçüm bandı hazır edilir.\n\nUygulama:\n1. Katılımcı 12 dakika boyunca mümkün olan en uzun mesafeyi koşar.\n2. Süre dolduğunda koşulan toplam mesafe ölçülür.\n3. Katılımcı istediği tempoda koşabilir, gerekirse yürüyebilir.\n\nSonuç ve Puanlama:\n- Sonuç, 12 dakikada koşulan toplam mesafe (metre) olarak kaydedilir.\n- VO2max tahmini için: VO2max = (koşulan mesafe - 504.9) / 44.73 (ml/kg/dk)',
    resultUnit: 'metre',
  ),
  // Anaerobik Testler
  TestDefinitionModel(
    id: 'margaria',
    name: 'Margaria-Kalamen Testi',
    category: 'Anaerobik',
    description: 'Basamak çıkma ile anaerobik gücü ölçer.',
    purpose: 'Basamak çıkma ile anaerobik gücü ölçer.',
    protocol: 'Parkur Hazırlığı:\n1. 9 basamaklı bir merdiven hazırlanır, 3. ve 9. basamaklar işaretlenir.\n2. Kronometre hazır edilir.\n\nUygulama:\n1. Katılımcı 6m mesafeden koşarak gelir, 3. basamaktan 9. basamağa en kısa sürede çıkar.\n2. Süre kaydedilir.\n\nSonuç ve Puanlama:\n- Yükseklik = 6 basamak × basamak yüksekliği (genellikle 15-20 cm)\n- Güç (Watt) = (Ağırlık × 9.81 × yükseklik) / süre\n- En iyi deneme kaydedilir.',
    resultUnit: 'Watt',
  ),
  // Güç/Patlayıcı Güç
  TestDefinitionModel(
    id: 'vertical-jump',
    name: 'Dikey Sıçrama (Vertical Jump)',
    category: 'Patlayıcı Güç',
    description: 'Dikey sıçrama testi (Sargent Jump), sporcunun ayakta durduğu yerden maksimum yükseklikte sıçrayarak alt vücut patlayıcı gücünü ölçen klasik bir testtir. 1921 yılında Dudley Sargent tarafından geliştirilmiştir.',
    purpose: 'Alt ekstremite patlayıcı gücünü, kas kuvvetini ve hızlı kasılma kapasitesini değerlendirmek için kullanılır. Basketbol, voleybol, atletizm gibi branşlarda sıçrama performansını izlemek için idealdir. NBA ve AFL draft testlerinde de kullanılmaktadır.',
    protocol: 'Ekipman Hazırlığı:\n1. Düz bir duvar, tebeşir veya özel ölçüm cihazı (Vertec®, jump mat) hazırlanır.\n2. Ölçüm bandı veya işaretleme sistemi kurulur.\n\nUygulama (Sargent Jump Protokolü):\n1. Sporcu duvara yan döner ve ayakta dururken parmak uçlarıyla duvara dokunur.\n2. Bu nokta "ayakta uzanma yüksekliği" olarak işaretlenir.\n3. Sporcu geri çekilir ve maksimum yükseklikte sıçrar.\n4. Sıçrama sırasında ulaştığı en yüksek nokta işaretlenir.\n5. 3 deneme yapılır, en iyi sonuç alınır.\n\nSonuç ve Puanlama:\n- Sıçrama yüksekliği = Sıçrama noktası - Ayakta uzanma yüksekliği (cm)\n- Güç hesaplama: P = 21.67 × Ağırlık × √Sıçrama Yüksekliği (Watt)\n- En iyi değer kaydedilir.\n\nVaryasyonlar:\n- Countermovement jump (diz bükülerek)\n- Squat jump (çömelik pozisyondan)\n- Tek bacak sıçrama\n- Koşarak sıçrama',
    resultUnit: 'cm',
  ),
  TestDefinitionModel(
    id: 'cmj',
    name: 'Countermovement Jump (CMJ)',
    category: 'Patlayıcı Güç',
    description: 'Countermovement Jump (CMJ), alt vücut patlayıcı gücünü ölçmek için kullanılan basit, pratik, geçerli ve son derece güvenilir bir testtir. Sprint performansı, 1RM maksimal kuvvet ve patlayıcı güç testleri ile ilişkilidir.',
    purpose: 'Alt ekstremite patlayıcı gücünü, nöromüsküler yorgunluğu ve antrenman adaptasyonlarını değerlendirmek için kullanılır. Futbol, basketbol, voleybol, rugby ve olimpik kaldırış gibi patlayıcı güç gerektiren sporlarda performans izleme için idealdir.',
    protocol: 'Ekipman Hazırlığı:\n1. Contact mat, force platform, infrared platform veya high-speed kamera sistemi hazırlanır.\n2. Tutarlı test ortamı sağlanır (gym veya laboratuvar).\n\nUygulama (CMJ Protokolü):\n1. Sporcu test platformuna ayakları omuz genişliğinde diker.\n2. Kısa bir "countermovement" (ön-germe) hareketi yapar.\n3. Maksimum yükseklikte sıçrar ve aynı noktaya iner.\n4. Uçuş sırasında kalça, diz ve ayak bileği ekstansiyonda tutulur.\n5. Minimum 3 deneme yapılır, en iyi sonuç alınır.\n\nKol Hareketi:\n- Kol hareketi ile: Performansı %10+ artırır\n- Kol hareketi olmadan: Eller kalçada tutulur\n\nSonuç ve Puanlama:\n- Sıçrama yüksekliği: h = (g × t²) ÷ 8 (m)\n- Güç hesaplama: P = (m × g × h × 60) ÷ t (Watt)\n- Uçuş süresi 0.3-1.0 saniye aralığında olmalıdır\n- En iyi değer kaydedilir.',
    resultUnit: 'cm',
  ),
  TestDefinitionModel(
    id: 'medicine-ball',
    name: 'Medicine Ball Throw (Seated)',
    category: 'Patlayıcı Güç',
    description: 'Medicine Ball Throw testi, üst vücut patlayıcı gücünü ve kas koordinasyonunu ölçen geçerli ve güvenilir bir testtir. NHL, US Army ve birçok spor organizasyonu tarafından kullanılmaktadır.',
    purpose: 'Üst vücut (kol, omuz, göğüs) patlayıcı gücünü değerlendirmek, antrenman ilerlemesini izlemek ve spor performansını değerlendirmek için kullanılır. Tenis, voleybol, basketbol, rugby gibi üst vücut gücü gerektiren branşlarda önemlidir.',
    protocol: 'Parkur Hazırlığı:\\n1. Düz duvar, 4kg medicine ball ve metre şeridi hazırlanır.\\n\\nUygulama:\\n1. Sporcu yere oturur, sırtı duvara dayalı kalır.\\n2. Bacaklar düz uzatılır, ayaklar 60cm aralıkta.\\n3. Medicine ball göğüste tutulur, ön kollar yere paralel.\\n4. Maksimum güçle düz ileri atış yapılır.\\n5. Sırt duvardan ayrılmadan tüm atış tamamlanır.\\n6. 1-2 dakika dinlenme ile 3 deneme yapılır.\\n\\nSonuç ve Puanlama:\\n- Duvardan topun ilk değdiği noktaya kadar mesafe ölçülür.\\n- En iyi deneme kaydedilir.\\n- Güç İndeksi = Mesafe × Ball Ağırlığı',
    resultUnit: 'm',
  ),
  // Çeviklik
  TestDefinitionModel(
    id: 'illinois',
    name: 'Illinois Çeviklik Testi',
    category: 'Çeviklik',
    description: 'Illinois çeviklik testi, 10x5 metrelik parkurda koniler arasında belirli bir rotada koşarak çeviklik ve yön değiştirme kabiliyetini ölçen klasik bir saha testidir.',
    purpose: 'Sporcunun kısa sürede yön değiştirme, hızlanma ve yavaşlama yeteneğini ölçer. Futbol, basketbol, hentbol gibi sporlarda çeviklik ve reaksiyon performansını değerlendirmek için kullanılır.',
    protocol: 'Parkur Hazırlığı:\n1. 10m uzunluğunda ve 5m genişliğinde bir parkur hazırlanır.\n2. 4 koni ortada, 4 koni köşelerde olacak şekilde dizilir.\n3. Kronometre hazır edilir.\n\nUygulama:\n1. Katılımcı yere yatar pozisyonda başlar.\n2. Başla komutuyla kalkıp parkuru belirlenen sırayla en kısa sürede tamamlar.\n3. Her deneme için kronometre kullanılır.\n\nSonuç ve Puanlama:\n- Sonuç, parkuru tamamlama süresi (saniye) olarak kaydedilir.\n- Daha kısa süre, daha iyi çeviklik anlamına gelir.',
    resultUnit: 'saniye',
  ),
  TestDefinitionModel(
    id: 't-test',
    name: 'T-Test',
    category: 'Çeviklik',
    description: 'T-Test, çok yönlü hareket ve çeviklik kabiliyetini ölçen NSCA tarafından standartlaştırılmış bir testtir. İleri, lateral ve geri koşu kombinasyonunu içerir. Semenick (1990) tarafından geliştirilen bu test, takım sporlarında performans değerlendirmesi için yaygın olarak kullanılır.',
    purpose: 'Sporcunun çok yönlü hareket kabiliyetini, yön değiştirme çevikliğini, lateral shuffle tekniğini ve geri koşu performansını ölçer. Futbol, basketbol, hentbol, hokej gibi çok yönlü hareket gerektiren sporlarda çeviklik performansını değerlendirmek için kullanılır.',
    protocol: 'Parkur Hazırlığı:\n1. T şeklinde 4 koni dizilir: A (başlangıç), B (10m ileri), C (5m sol), D (5m sağ)\n2. Mesafeler: A-B: 10m, B-C: 5m, B-D: 5m\n3. Kronometre veya timing gates hazırlanır\n\nUygulama (NSCA Standardı):\n1. Sporcu A konisinde başlar, B konisine sprint yapar ve sağ eliyle dokunur\n2. B\'den C konisine shuffle yapar (ayaklar çaprazlanmaz), sol eliyle dokunur\n3. C\'den D konisine shuffle yapar (10m), sağ eliyle dokunur\n4. D\'den B konisine shuffle yapar (5m), sol eliyle dokunur\n5. B\'den A konisine geri koşu yapar (yüz öne dönük)\n6. Tüm test boyunca öne bakış zorunlu\n\nKurallar:\n- Shuffle sırasında ayaklar çaprazlanmaz\n- Her koni elle dokunulmalı\n- Sürekli öne bakış\n- 3 deneme, en iyi sonuç alınır\n\nSonuç ve Puanlama:\n- Erkekler için norm değerleri: Mükemmel <9.5s, İyi 9.5-10.5s, Orta 10.5-11.5s\n- Kadınlar için norm değerleri: Mükemmel <10.5s, İyi 10.5-11.5s, Orta 11.5-12.5s',
    resultUnit: 'saniye',
  ),
  TestDefinitionModel(
    id: 'pro-agility',
    name: '5-10-5 Pro Agility Test',
    category: 'Çeviklik',
    description: '5-10-5 Pro Agility Test (20 Yard Shuttle), NFL, NHL ve MLS Combine testlerinde kullanılan resmi çeviklik testidir. SPARQ değerlendirme sisteminin temel bileşenidir. Sporcunun hız, patlama gücü, vücut kontrolü ve yön değiştirme kabiliyetini ölçer.',
    purpose: 'Sporcunun kısa mesafede hızla yön değiştirme, hızlanma ve durma kabiliyetini değerlendirmek için kullanılır. Amerikan futbolu, basketbol, hokey, beyzbol gibi ani yön değişikliği gerektiren sporlar için kritik bir performans göstergesidir.',
    protocol: 'Parkur Hazırlığı:\n1. 3 koni düz bir çizgide 5 yard aralıklarla dizilir (toplam 10 yard)\n2. Kronometre veya timing gates hazırlanır\n3. Kaymaz zemin ve açık alan gereklidir\n\nUygulama (NFL Combine Standardı):\n1. Sporcu orta çizgide 3-point stance ile başlar\n2. İstediği yönde 5 yard sprint yapar, eliyle çizgiye dokunur\n3. Ters yöne döner ve 10 yard sprint yapar, diğer çizgiye eliyle dokunur\n4. Tekrar döner ve 5 yard sprint yaparak başlangıç çizgisini geçer\n5. Toplam mesafe: 5+10+5 = 20 yard (18.3 metre)\n\nKurallar:\n- Her çizgiye el ile dokunma zorunlu\n- 3-point stance başlangıç pozisyonu\n- Koordinasyon ve dönüş tekniği önemli\n- 3 deneme, en iyi sonuç alınır\n\nSonuç ve Puanlama:\n- NFL Combine norm değerleri: Elit <4.0s, İyi 4.0-4.5s, Orta 4.5-5.0s\n- US Decathlete Bryan Clay: 4.38s (SPARQ Magazine 2008)\n- Sonuç 0.01 saniye hassasiyetle kaydedilir',
    resultUnit: 'saniye',
  ),
  // Sürat
  TestDefinitionModel(
    id: '10m-sprint',
    name: '10m Sprint',
    category: 'Sürat',
    description: '10 metre sprint testi, sporcunun kısa mesafede maksimum hızlanma ve sürat kapasitesini ölçen basit bir hız testidir.',
    purpose: 'Kısa mesafede patlayıcı hız ve reaksiyon süresini değerlendirmek için kullanılır. Özellikle futbol, atletizm, basketbol gibi branşlarda ilk adım ve hızlanma performansını izlemek için uygundur.',
    protocol: 'Parkur Hazırlığı:\n1. 10 metrelik düz bir parkur hazırlanır.\n2. Kronometre veya fotosel sistemi kurulur.\n\nUygulama:\n1. Katılımcı başlangıç çizgisinden başlar.\n2. 10 metreyi en kısa sürede koşar.\n3. Süre kaydedilir.\n\nSonuç ve Puanlama:\n- Sonuç, 10 metreyi tamamlama süresi (saniye) olarak kaydedilir.\n- Daha kısa süre, daha iyi sürat anlamına gelir.',
    resultUnit: 'saniye',
  ),
  TestDefinitionModel(
    id: '20m-sprint',
    name: '20m Sprint',
    category: 'Sürat',
    description: 'Kısa mesafede hız ölçümü.',
    purpose: 'Kısa mesafede hız ölçümü.',
    protocol: 'Parkur Hazırlığı:\n1. 20 metrelik düz bir parkur hazırlanır.\n2. Kronometre veya fotosel sistemi kurulur.\n\nUygulama:\n1. Katılımcı başlangıç çizgisinden başlar.\n2. 20 metreyi en kısa sürede koşar.\n3. Süre kaydedilir.\n\nSonuç ve Puanlama:\n- Sonuç, 20 metreyi tamamlama süresi (saniye) olarak kaydedilir.\n- Daha kısa süre, daha iyi sürat anlamına gelir.',
    resultUnit: 'saniye',
  ),
  TestDefinitionModel(
    id: '30m-sprint',
    name: '30m Sprint',
    category: 'Sürat',
    description: 'Kısa mesafede hız ölçümü.',
    purpose: 'Kısa mesafede hız ölçümü.',
    protocol: 'Parkur Hazırlığı:\n1. 30 metrelik düz bir parkur hazırlanır.\n2. Kronometre veya fotosel sistemi kurulur.\n\nUygulama:\n1. Katılımcı başlangıç çizgisinden başlar.\n2. 30 metreyi en kısa sürede koşar.\n3. Süre kaydedilir.\n\nSonuç ve Puanlama:\n- Sonuç, 30 metreyi tamamlama süresi (saniye) olarak kaydedilir.\n- Daha kısa süre, daha iyi sürat anlamına gelir.',
    resultUnit: 'saniye',
  ),
  // Esneklik
  TestDefinitionModel(
    id: 'sit-reach',
    name: 'Sit and Reach (Otur-Uzan) Testi',
    category: 'Esneklik',
    description: 'Sit and Reach testi, katılımcının oturur pozisyonda öne doğru uzanabildiği maksimum mesafeyi ölçerek bel ve hamstring esnekliğini değerlendiren yaygın bir esneklik testidir.',
    purpose: 'Bel ve hamstring kaslarının esnekliğini, sakatlık riskini ve genel hareket kabiliyetini değerlendirmek için kullanılır. Özellikle jimnastik, dans, atletizm gibi branşlarda önemlidir.',
    protocol: 'Parkur Hazırlığı:\n1. Otur-uzan kutusu veya cetvel hazırlanır.\n\nUygulama:\n1. Katılımcı ayak tabanları kutuya dayalı şekilde oturur.\n2. Eller üst üste, öne doğru en uzağa uzanır.\n3. Uzanılan mesafe kutu veya cetvel üzerinden okunur.\n\nSonuç ve Puanlama:\n- Sonuç, uzanılan mesafe (cm) olarak kaydedilir.\n- Daha uzun mesafe, daha iyi esneklik anlamına gelir.',
    resultUnit: 'cm',
  ),
  TestDefinitionModel(
    id: 'shoulder-flex',
    name: 'Shoulder Flexibility Test',
    category: 'Esneklik',
    description: 'Omuz eklemi esnekliğini ölçer.',
    purpose: 'Omuz eklemi esnekliğini ölçer.',
    protocol: 'Parkur Hazırlığı:\n1. 1 metre uzunluğunda bir çubuk hazırlanır.\n\nUygulama:\n1. Katılımcı çubuğu iki el ile tutar.\n2. Kollar düz, çubuk baş üzerinden arkaya doğru götürülür.\n3. Eller arası mesafe ölçülür.\n\nSonuç ve Puanlama:\n- Sonuç, eller arası mesafe (cm) olarak kaydedilir.\n- Daha kısa mesafe, daha iyi omuz esnekliği anlamına gelir.',
    resultUnit: 'cm',
  ),
  // Dayanıklılık
  // Bu bölümde test bulunmuyor
  // Saha/İndirekt Testler
  TestDefinitionModel(
    id: 'astrand',
    name: 'Astrand-Rhyming Cycle Ergometer Test',
    category: 'İndirekt (Submaksimal)',
    description: 'Bisiklet ergometresi ile submaksimal VO2max tahmini.',
    purpose: 'Bisiklet ergometresi ile submaksimal VO2max tahmini.',
    protocol: 'Ekipman:\n- Bisiklet ergometresi, nabız ölçer, kronometre.\n\nUygulama:\n1. Katılımcı 6 dakika sabit tempoda bisiklet çevirir.\n2. Erkek: 600 kgm/dk (100 Watt)\n3. Kadın: 450 kgm/dk (75 Watt)\n4. Son 2 dakikada nabız sabitlenmiş olmalı (fark < 5 bpm).\n\nSonuç ve Puanlama:\n- VO2max, yük ve nabız değerlerine göre özel tablo veya formülle hesaplanır.\n- VO2max = (Yük × 1000) / (Nabız × 6.3) × 1.1 (ml/kg/dk)',
    resultUnit: 'ml/kg/dk',
  ),
  TestDefinitionModel(
    id: 'balke-test',
    name: 'Balke Test',
    category: 'Kardiyovasküler',
    description: 'Balke Test, sporcuların kardiyovasküler dayanıklılığını ölçmek için kullanılan bir koşu testidir. 15 dakika süreyle sabit hızda koşarak maksimum mesafeyi kat etmeye dayalıdır.',
    purpose: 'Kardiyovasküler dayanıklılığı ölçmek, VO2max tahmini yapmak ve sporcuların genel fitness seviyesini değerlendirmek için kullanılır. Özellikle atletizm, futbol, basketbol gibi dayanıklılık gerektiren sporlarda önemlidir.',
    protocol: 'Parkur Hazırlığı:\n1. 400 metrelik bir koşu pisti hazırlanır.\n2. Kronometre ve ölçüm bandı hazır edilir.\n\nUygulama:\n1. Katılımcı 15 dakika süreyle maksimum hızda koşar.\n2. Hız sabit tutulmaya çalışılır.\n3. Koşulan toplam mesafe ölçülür.\n4. Test süresi 15 dakika ile sınırlıdır.\n\nSonuç ve Puanlama:\n- Sonuç, 15 dakikada koşulan toplam mesafe (metre) olarak kaydedilir.\n- VO2max tahmini için: VO2max = (mesafe × 0.0084) + 36.4 (ml/kg/dk)\n- Daha uzun mesafe, daha iyi kardiyovasküler dayanıklılık anlamına gelir.',
    resultUnit: 'metre',
  ),
  TestDefinitionModel(
    id: 'bruce-protocol',
    name: 'Bruce Protokolü',
    category: 'Kardiyovasküler',
    description: 'Bruce Protokolü, sporcuların kardiyovasküler dayanıklılığını ve VO2max değerini ölçmek için kullanılan bir koşu bandı testidir. Artan hız ve eğimle maksimum efora kadar devam eder.',
    purpose: 'Kardiyovasküler dayanıklılığı ölçmek, VO2max değerini hesaplamak ve sporcuların maksimum efor kapasitesini değerlendirmek için kullanılır. Özellikle atletizm, futbol, basketbol gibi dayanıklılık gerektiren sporlarda önemlidir.',
    protocol: 'Ekipman Hazırlığı:\n1. Koşu bandı, kronometre ve nabız ölçer hazırlanır.\n2. Test ortamı uygun sıcaklık ve nemde tutulur.\n\nUygulama:\n1. Katılımcı koşu bandı üzerinde başlangıç hızı 2.7 km/saat ile başlar.\n2. Her 3 dakikada bir hız ve eğim artırılır.\n3. Test maksimum efora kadar devam eder.\n4. Tamamlama süresi kaydedilir.\n5. VO2max değeri hesaplanır.\n\nSonuç ve Puanlama:\n- Sonuç, test tamamlama süresi (dakika) olarak kaydedilir.\n- VO2max hesaplama: VO2max = (4.38 × süre) - 3.9 (ml/kg/dk)\n- Daha uzun süre, daha iyi kardiyovasküler dayanıklılık anlamına gelir.',
    resultUnit: 'dakika',
  ),
]; 